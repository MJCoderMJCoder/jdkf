...
<style type="text/css">
.Header {
	background: url(<%=path%>/img/banner.jpg) #d10e00 repeat-x left top;
	height: 70px;
	width: 966px;
}
...
<body>
	<div class="Header HeaderTop">
		<br /> <font
			style="font-size: 40px; color: white; font-weight: bolder; display: block; text-align: center;">酒店管理系统</font>
	</div>
		...
		<ul>
			...
			<c:forEach items="${sessionScope.cateLogList}" var="catelog">
				<li class="thisclass"><A
					href="<%=path %>/kefangByCatelog.action?catelogId=${catelog.catelogId}">${catelog.catelogName}</A>
				</li>
			</c:forEach>
			<li class="thisclass"><A href="#" onclick="reg()">在线注册</A></li>
			<li class="thisclass"><A href="#" onclick="myxinxi()">我的信息</A></li>
			<li class="thisclass"><A href="#" onclick="liuyanAll()">留言板</A>
			</li>
		</ul>
	</div>
	<form id="searchForm" action="<%=path%>/kefangSearch.action"
		method="post">
		<div class="topsearch">
			<div class="title"></div>
			<div id="page_search_left">
				价格区间： <input onfocus="this.value=''" class="inputText" id=""
					size="5" name="qianshu1" type="text" value="50"
					onpropertychange="onchange1(this.value)"
					onkeyup="this.value=this.value.replace(/\D/g,'')"
					onafterpaste="this.value=this.value.replace(/\D/g,'')" /> -- <input
					onfocus="this.value=''" class="inputText" id="" size="5"
					name="qianshu2" type="text" value="1000"
					onpropertychange="onchange1(this.value)"
					onkeyup="this.value=this.value.replace(/\D/g,'')"
